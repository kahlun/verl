        ...
        if parallel_dims.pp_enabled:
            ...
            with self.trainer.train_context():
                if self.trainer.pp_has_first_stage:
                    outputs = self.trainer.pp_schedule.step(inputs, **extra_inputs, **extra_kwargs, return_outputs=True)
                else:
...
class TorchTitanEngineWithLMHead(TorchTitanEngine):
        ...
        if use_remove_padding:
            ...
            if self.parallel_dims.tp_enabled:
                ...
                if remainder != 0:
                    ...
                    if position_ids.dim() == 3:
                        pad_pos = torch.zeros(
                            1, position_ids.shape[1], tp_pad_len, dtype=position_ids.dtype, device=position_ids.device
                        )
    ...
    def _pp_forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False):
        ...
        if forward_only:
            ...
            total_loss = torch.sum(torch.stack(losses_list)).item() if (losses_list and has_last) else 0.0
