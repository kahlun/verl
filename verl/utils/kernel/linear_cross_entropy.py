...
class LinearCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        with (torch.cuda.nvtx.range("LinearCrossEntropy-forward") if torch.cuda.is_available() else contextlib.nullcontext()):
            from . import kernels
    ...
    def backward(ctx, dlogprobs: torch.Tensor, dentropy: torch.Tensor) -> list[torch.Tensor]:
        ...
        with (torch.cuda.nvtx.range("LinearCrossEntropy-backward") if torch.cuda.is_available() else contextlib.nullcontext()):
