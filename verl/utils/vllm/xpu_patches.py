...
def _patch_request_memory() -> bool:
    ...
    try:
        ...
        if not hasattr(_utils, "request_memory"):
            logger.warning("[XPU patch] vllm.v1.worker.utils.request_memory not found — "
                           "skipping Patch 1 (API may have changed)")
            return False
...
def _patch_profiling_assert() -> bool:
    ...
    *increases*, tripping the assert even though this is expected behaviour.
    ...
    try:
        ...
        if not patched_any:
            logger.warning(f"[XPU patch] None of {_TARGET_METHODS} found on GPUWorker — "
...
def apply() -> None:
    ...
    try:
        import torch  # noqa: F401 — confirm torch present before attempting vLLM
    except ImportError:
