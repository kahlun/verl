...
def get_default_attention_implementation() -> str:
    ...
    On XPU the priority is:
      1. ``flash_attention_2`` — if ``kernels-community/flash-attn2`` is loadable
         (routes through HF Transformers kernel dispatch, ~2–15x faster than SDPA
         on B60; see https://github.com/huggingface/kernels-community/pull/671).
...
def resolve_xpu_attn_implementation(attn_implementation: str, model_config=None) -> str:
    ...
    On XPU, ``flash_attention_2`` with sliding-window models is unsupported;
    falls back to ``sdpa`` automatically.

    Args:
        attn_implementation: the requested implementation name.
        model_config: a HF ``PretrainedConfig`` (optional) — used to check
            ``sliding_window``.
    ...
    if attn_implementation == "flash_attention_2" and model_config is not None:
        ...
        if sw is not None and sw > 0:
            logger.warning(
                f"[XPU] Model has sliding_window={sw} which is unsupported with "
                "flash_attention_2 on XPU. Falling back to sdpa."
            )
            return "sdpa"
