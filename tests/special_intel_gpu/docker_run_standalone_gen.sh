#!/usr/bin/env bash
# Launch the standalone generation-server test (run_standalone_gen_intel_gpu.sh)
...
#
# Usage:
#   NUM_GPUS=2 bash tests/special_intel_gpu/docker_run_standalone_gen.sh
...
docker run --rm --name verl_standalone_gen_test \
    ...
    bash -c '
        ...
        cd /workspace/verl
        NUM_GPUS='"${NUM_GPUS}"' bash tests/special_intel_gpu/run_standalone_gen_intel_gpu.sh
    '
