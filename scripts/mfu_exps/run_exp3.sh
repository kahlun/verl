#!/usr/bin/env bash
# MFU Experiment 3: AutoModel 4-GPU Llama-3.1-8B TP=1 (FSDP4 via HSDP)
set -euo pipefail
...
CKPT_DIR=/root/verl/ckpts/mfu-am-llama8b-tp1
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --standalone --nnodes=1 --nproc-per-node=4 \
  ...
  trainer.experiment_name=am-llama8b-tp1 \
  trainer.total_epochs=999 \
  trainer.total_training_steps=30 \
  trainer.default_local_dir="${CKPT_DIR}" \
  trainer.resume_mode=disable \
  2>&1 | tee "${LOG_DIR}/mfu-am-llama8b-tp1.log"
