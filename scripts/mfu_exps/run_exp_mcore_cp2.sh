#!/usr/bin/env bash
# MFU Experiment: Megatron 2-GPU Llama-3.1-8B CP=2, DP=1
...
CKPT_DIR=/root/verl/ckpts/mfu-mcore-llama8b-cp2
...
CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nnodes=1 --nproc-per-node=2 \
  ...
  trainer.experiment_name=mcore-llama8b-cp2 \
  trainer.total_epochs=999 \
  trainer.total_training_steps=30 \
  trainer.default_local_dir="${CKPT_DIR}" \
  trainer.resume_mode=disable \
  2>&1 | tee "${LOG_DIR}/exp_mcore_cp2.log"
