#!/usr/bin/env bash
# MFU Experiment: Megatron 4-GPU Llama-3.1-8B TP=1, DP=4 (baseline)
set -euo pipefail
...
CKPT_DIR=/root/verl/ckpts/mfu-mcore-llama8b-tp1-dp4
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --standalone --nnodes=1 --nproc-per-node=4 \
  ...
  trainer.experiment_name=mcore-llama8b-tp1-dp4 \
  trainer.total_epochs=999 \
  trainer.total_training_steps=30 \
  trainer.default_local_dir="${CKPT_DIR}" \
  trainer.resume_mode=disable \
  2>&1 | tee "${LOG_DIR}/exp_mcore_tp1.log"
