#!/usr/bin/env bash
# MFU Experiment: Megatron 2-GPU Qwen1.5-MoE-A2.7B EP=2, DP=1
# Host GPUs 1,4 → Container GPUs 0,1
# 60 experts / EP=2 = 30 experts per GPU
set -euo pipefail

MODEL_PATH=Qwen/Qwen1.5-MoE-A2.7B
DATA_DIR=/root/data/gsm8k_sft
LOG_DIR=/workspace/verl/mfu_comparison
CKPT_DIR=/root/verl/ckpts/mfu-mcore-qwen15moe-ep2
...
CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nnodes=1 --nproc-per-node=2 \
  ...
  trainer.experiment_name=mcore-qwen15moe-ep2 \
  trainer.total_epochs=999 \
  trainer.total_training_steps=30 \
  trainer.default_local_dir="${CKPT_DIR}" \
  trainer.resume_mode=disable \
  2>&1 | tee "${LOG_DIR}/exp_mcore_ep2.log"
