#!/usr/bin/env bash
# MFU Experiment 4b: FSDP 1-GPU Llama-3.1-8B (single GPU baseline)
# Uses gradient_checkpointing to fit 8B in 80GB
set -euo pipefail
...
CKPT_DIR=/root/verl/ckpts/mfu-fsdp-llama8b-1gpu
...
CUDA_VISIBLE_DEVICES=0 torchrun --standalone --nnodes=1 --nproc-per-node=1 \
  ...
  trainer.experiment_name=fsdp-llama8b-1gpu \
  trainer.total_epochs=999 \
  trainer.total_training_steps=30 \
  trainer.default_local_dir="${CKPT_DIR}" \
  trainer.resume_mode=disable \
  2>&1 | tee "${LOG_DIR}/exp4b.log"
